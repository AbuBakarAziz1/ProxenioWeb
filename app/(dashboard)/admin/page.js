...
const Dashboard = () => {
  ...
  const userStatCanvasRef = useRef(null);
  const revenueChartInstance = useRef(null);
  const userStatChartInstance = useRef(null);
  ...
  useEffect(() => {
    ...
    if (userStatChartInstance.current) {
      userStatChartInstance.current.destroy();
    }
    if (userStatCanvasRef.current) {
      const ctx = userStatCanvasRef.current.getContext("2d");
      userStatChartInstance.current = new Chart(ctx, {
        type: "doughnut",
        data: {
          labels: ["Male", "Female"],
          datasets: [{
            backgroundColor: ["#CCEABB", "#FDCB9E"],
            data: [stats.userStats.male, stats.userStats.female],
          }],
        },
      });
    }
  ...
  return (
    <div className="row">
      <div className="col-md-8">
        ...
        <div className="row mb-3" style={{ height: "300px" }}>
      ...
      <div className="col-md-4">
        ...
        <canvas ref={userStatCanvasRef} style={{ width: "100%", maxWidth: "250px" }}></canvas>
      </div>
    </div>
  );
};
