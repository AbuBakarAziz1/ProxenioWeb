...
export default function ModeratorLayout({ children }) {
    ...
    const pathSegments = pathname
        .split("/")
        .filter((segment) => segment && segment !== "moderator" && !/^[a-fA-F0-9]{24}$/.test(segment));
...
function Breadcrumb({ pathSegments }) {
    return (
        <nav>
            <Link href="/moderator" className="text-muted text-decoration-none">Moderator </Link> {" >> "}
            {pathSegments.map((segment, index) => {
                const href = `/moderator/${pathSegments.slice(0, index + 1).join("/")}`;
                return (
                    <span key={index}>
